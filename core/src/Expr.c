...
Expr Expr_createByNameInternal(char *string){
	//fprintf(stderr,"Expr_create\n");
	Expr expr = allocate(sizeof(Expr_Sub));
	expr->symbol = SymbolTable_getByNameInternal(string);
	/*rational,complex is created by combining integer or real numbers. some function will do this*/
	/*you should set all connections coherent.*/
	/*don't make situations like following, "parent's parent is me." "next's previous is not me" ...etc*/
	expr->flag = 0;
...
Expr Expr_deleteLast(Expr expr){
	Expr e,last;
	if(!expr){return expr;}
...
Expr Expr_swapChild(Expr expr, Expr c1, Expr c2){
	Expr e,ee,c1p,c1n,c2p,c2n;
	char ordered=0;
	if(!expr){return NULL;}
	for(e=expr->child;e;e=e->next){
		if(e==c1){
			for(ee=expr->child;ee;ee=ee->next){
				if(ee==c2){
					if(c1==c2){return expr;}
