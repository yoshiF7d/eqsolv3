...
void Symbol_print_info(Symbol s){
	printf("Symbol @ %x\n",(unsigned long)s);
	printf("id : %d @ %x\n",s->id,(unsigned long)s->id);
	printf("name : %s @ %x\n",s->name,(unsigned long)s->name);
	printf("sign : %s @ %x\n",s->sign,(unsigned long)s->sign);
	printf("function @ %x\n",(unsigned long)s->function);
	printf("attributes @ %x",(unsigned long)s->attributes);
	if(s->attributes){
		printf(" : ");
		printf(KYEL); printAttributes(s->attributes); printf(KNRM);
	}else{putchar('\n');}
	printf("associativity : %d @ %x\n",s->associativity,(unsigned long)s->associativity);
	printf("precedence : %d @ %x\n",s->precedence,(unsigned long)s->precedence);
	printf("buitinrules @ %x :\n",(unsigned long)s->builtinrules);
	TreeForm_modh(s->builtinrules);
	printf("staticfield @ %x\n",(unsigned long)s->staticfield);
	printf("assigned @ %x : \n",(unsigned long)s->assigned);
	TreeForm_modh(s->assigned);
	printf("alias @ %x : \n",(unsigned long)s->alias);
...
void Symbol_vprint_info(void *s){Symbol_print_info(s);}
...
void Context_print_debug(Context c){
	printf(KRED "%s\n" KNRM,c->name);
	printf(LONGBAR128);
	HashTable_enumSorted(c->table,Symbol_vprint_info,Symbol_vcomp);
	printf("\n");
}
...
void Context_printall_debug(Context c){
	Context cc;
	for(cc=c;cc;cc=cc->next){
		Context_print_debug(cc);
	}
}

void Context_deleteField(Context c){
...
void Context_deleteFieldRoot(Context c){
	Context next;
	for(;c;c=next){next=c->next;Context_deleteField(c);}
}
...
Context SymbolTable_mod(char *name, enum en_stm_mode mode, SymbolTable *table ,Symbol *symbol, void (*doc)(int),int id,char ***nt){
	...
	switch(mode){
	  ...
	  case stm_init_internal:
		...
		for(c=context;c;c=c->next){
			if(c==user){continue;}
			parser_setfunc(Expr_createByNameInternal);
	  ...
	  case stm_get_by_name:
		...
		break;
	  case stm_get_by_name_internal:
		for(c=context;c;c=c->next){
			if((*symbol=HashTable_get(c->table,name))!=NULL){break;}
		}
		if(*symbol==NULL){
			if(alias){*symbol=Alias_getSymbol(HashTable_get(alias,name));}
		}
	  ...
	  case stm_print:
		...
		break;
	  case stm_printall_debug:
		Context_printall_debug(context);
		break;
	  case stm_print_debug:
		s=NULL;
		for(c=context;c;c=c->next){
			if((s=HashTable_get(c->table,name))!=NULL){
				break;
	  ...
	  case stm_finish:
		...
		Context_deleteFieldRoot(context);
...
Symbol SymbolTable_getByNameInternal(char *name){
	Symbol sd=NULL;
	SymbolTable_mod(name,stm_get_by_name_internal,NULL,&sd,NULL,0,NULL);
	return sd;
}
...
void SymbolTable_printall_debug(){
	SymbolTable_mod(NULL,stm_printall_debug,NULL,NULL,NULL,0,NULL);
}

void SymbolTable_print_debug(Symbol s){
	SymbolTable_mod(s->name,stm_print_debug,NULL,NULL,NULL,0,NULL);
}
